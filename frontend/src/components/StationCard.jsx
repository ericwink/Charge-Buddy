...
export default function StationCard({ stationInfo }) {
    ...
    const addToFavorites = async () => {
        try {
            const response = await axios.post('/user/addfavorite', {
                evID: stationInfo.id,
            ...
            <Card>
                <Card.Body>
                    ...
                    <Card.Subtitle className='d-flex justify-content-between'><small className="text-muted mt-2">Access: {stationInfo.access_code}</small>
                        {loggedInUser ? <a className={isFav ? 'favorite' : 'notfavorite'} onClick={() => addToFavorites()}><i class={isFav ? 'fa-solid fa-heart' : 'fa-regular fa-heart'}></i></a>
                            : null}
                    </Card.Subtitle>
